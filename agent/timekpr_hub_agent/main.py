...
def _apply_policy_push(enforcer: TimekprEnforcer, username: str, policy: dict) -> bool:
    ...
    ok &= enforcer.set_time_limit_for_days(
    ...
    ok &= enforcer.set_track_inactive(username, bool(policy.get("track_inactive", False)))
    ok &= enforcer.set_hide_tray_icon(username, bool(policy.get("hide_tray_icon", False)))
    ok &= enforcer.set_lockout_type(
        username,
        policy.get("lockout_type") or "lock",
        policy.get("wake_from") or "",
        policy.get("wake_to") or "",
    )
    ok &= _apply_allowed_hours(enforcer, username, policy.get("allowed_hours") or {})
    ok &= _apply_playtime(enforcer, username, policy.get("playtime") or {})
    return ok


def _apply_allowed_hours(enforcer: TimekprEnforcer, username: str, allowed_hours: dict) -> bool:
    ...
    for day, intervals in allowed_hours.items():
        ...
        ok &= enforcer.set_allowed_hours(username, str(day), payload)
    return ok


def _apply_playtime(enforcer: TimekprEnforcer, username: str, playtime: dict) -> bool:
    if not playtime:
        return True
    ok = enforcer.set_playtime_enabled(username, bool(playtime.get("enabled", False)))
    ok &= enforcer.set_playtime_limit_override(username, bool(playtime.get("override_enabled", False)))
    ok &= enforcer.set_playtime_unaccounted_intervals_enabled(
        username, bool(playtime.get("unaccounted_intervals_enabled", True))
    )
    pt_weekdays = playtime.get("allowed_weekdays") or _ALL_WEEKDAYS
    ok &= enforcer.set_playtime_allowed_days(username, pt_weekdays)
    pt_daily_limits = [int(x) for x in (playtime.get("daily_limits_s") or [0] * 7)]
    ok &= enforcer.set_playtime_limits_for_days(
    ...
    ok &= enforcer.set_playtime_activities(username, activities)
    return ok
