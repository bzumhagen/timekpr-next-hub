...
PLAN reference: "Parent auth" -- password + session cookie (no TOTP for
Phase 1; `Parent.totp_secret` stays unused until there's demand for it).
Sessions mirror the device-token pattern in `api/auth.py` deliberately,
